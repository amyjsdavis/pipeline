...
indicator_total_year <- function(df, start_year_plot = 1940, 
    ...
    # Provide warning messages for endDate/startDate NA values
    if (nrow(filter(df, is.na(startDate), is.na(endDate))) > 0) {
        warning("Some records have no startDate or endDate defines, ",
                "these are currently not taken into account")
    }        
    if (nrow(filter(df, !is.na(startDate), is.na(endDate))) > 0) {
        warning("Some records have a startDate, but no endDate defines, ",
                "using same year as startYear to replace NA")
    }
    df_cleaned <- df_cleaned %>%
        filter(!is.na(startDate)) %>%  # ignore information without startDate
        mutate(endDate = if_else(is.na(endDate), startDate, endDate)) # replace NA with startDate
